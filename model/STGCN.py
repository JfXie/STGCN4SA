...
'''
Model code of STGCN.
refer to https://github.com/ziyujia/MSTGCN 
--------
Model input:  (*, T, V, F)
    T: num_of_timesteps
    V: num_of_vertices
...
def STGCN_Block(x, k, num_of_chev_filters, num_of_time_filters, time_conv_strides,
...
def build_STGCN(k, num_of_chev_filters, num_of_time_filters, time_conv_strides, cheb_polynomials,
                 time_conv_kernel, sample_shape, num_block, dense_size, opt, GLalpha,
                 regularizer, dropout, lambda_reversal, num_classes=1):

    # Input:  (*, num_of_timesteps, num_of_vertices, num_of_features)
    data_layer = layers.Input(shape=sample_shape, name='Input_Layer')

    # STGCN_Block
    block_out_GL, block_out_SD = STGCN_Block(data_layer, k, num_of_chev_filters, num_of_time_filters,
                                              time_conv_strides, cheb_polynomials, time_conv_kernel, GLalpha)
    for i in range(1, num_block):
        block_out_GL, block_out_SD = STGCN_Block(block_out_GL, k, num_of_chev_filters, num_of_time_filters,
    ...
    sigmoid = layers.Dense(num_classes,
                           activation='sigmoid',
                           kernel_regularizer=regularizer,
                           name='Label')(dense_out)
    ...
    # # training model (with GRL & G_d)
    # model = models.Model(inputs=data_layer, outputs=[sigmoid, G_d_out])
    # model.compile(
    #     optimizer=opt,
    #     loss='categorical_crossentropy',
    #     metrics=['acc'],
    # )
    # testing model (without GRL & G_d)
    pre_model = models.Model(inputs=data_layer, outputs=sigmoid)
    pre_model.compile(
        optimizer=opt,
...
def build_STGCN_test():
    ...
    model = build_STGCN(cheb_k, num_of_chev_filters, num_of_time_filters, time_conv_strides, cheb_polynomials,
    ...
    model.summary()
    model.save('STGCN_build_test.h5')
    print("save ok")
    return model
